...
// RossoneroApp.mc), NOT a wrapper View that itself pushes a menu.
...
class RossoneroSettingsMenu extends WatchUi.Menu2 {
    function initialize() {
...
class RossoneroSettingsDelegate extends WatchUi.Menu2InputDelegate {
    function initialize() {
    ...
    // RossoneroView.mc's constants and settings.xml's Field1/2/3 list
    ...
    function pushFieldPicker(parentItem as WatchUi.MenuItem, propKey as Lang.String) as Void {
        ...
        WatchUi.pushView(menu, new RossoneroFieldPickerDelegate(parentItem, propKey), WatchUi.SLIDE_IMMEDIATE);
    }

    // Whole-hour UTC offsets only, same range as settings.xml's
    // WorldClockOffset list (-12..+14) - see RossoneroView.mc's
    ...
    function pushWorldClockPicker(parentItem as WatchUi.MenuItem) as Void {
        ...
        WatchUi.pushView(menu, new RossoneroWorldClockPickerDelegate(parentItem), WatchUi.SLIDE_IMMEDIATE);
    }

    // Digital (the original hour:minute readout) vs Analog (hour/minute
    // hands from screen center) - see RossoneroView.mc's drawAnalogTime()
    ...
    function pushClockStylePicker(parentItem as WatchUi.MenuItem) as Void {
        ...
        WatchUi.pushView(menu, new RossoneroClockStylePickerDelegate(parentItem), WatchUi.SLIDE_IMMEDIATE);
    }
}

class RossoneroFieldPickerDelegate extends WatchUi.Menu2InputDelegate {
...
class RossoneroWorldClockPickerDelegate extends WatchUi.Menu2InputDelegate {
    private var _parentItem as WatchUi.MenuItem;
...
class RossoneroClockStylePickerDelegate extends WatchUi.Menu2InputDelegate {
    private var _parentItem as WatchUi.MenuItem;
